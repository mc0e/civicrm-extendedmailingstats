...
require_once 'mailingstats.civix.php';

/**
 * Implementation of hook_civicrm_config
 */
function mailingstats_civicrm_config(&$config) {
  _mailingstats_civix_civicrm_config($config);
}
 ...
 */
function mailingstats_civicrm_xmlMenu(&$files) {
  _mailingstats_civix_civicrm_xmlMenu($files);
}

/**
 * Implementation of hook_civicrm_install
 */
function mailingstats_civicrm_install() {
  return _mailingstats_civix_civicrm_install();
}

/**
 * Implementation of hook_civicrm_uninstall
 */
function mailingstats_civicrm_uninstall() {
  return _mailingstats_civix_civicrm_uninstall();
}

/**
 * Implementation of hook_civicrm_enable
 */
function mailingstats_civicrm_enable() {
  return _mailingstats_civix_civicrm_enable();
}

/**
 * Implementation of hook_civicrm_disable
 */
function mailingstats_civicrm_disable() {
  return _mailingstats_civix_civicrm_disable();
}
 ...
 */
function mailingstats_civicrm_upgrade($op, CRM_Queue_Queue $queue = NULL) {
  return _mailingstats_civix_civicrm_upgrade($op, $queue);
}
 ...
 */
function mailingstats_civicrm_managed(&$entities) {
  return _mailingstats_civix_civicrm_managed($entities);
}
 ...
 */
function _mailingstats_cron($params) {
//  _mailingstats_cron_db_setup($params, 1);
  _mailingstats_cron_mailing($params); // needs to be done at the start
  _mailingstats_cron_job_timing($params);
  _mailingstats_cron_recipients($params, 'Collect number of recipients',0);
  _mailingstats_cron_recipients($params, 'Collect number of Gmail recipients',1);

  _mailingstats_cron_event($params, 'delivered', 
    array (
  ...
  _mailingstats_cron_event($params, 'gmail_delivered',
    array (
      'event_type' => "civicrm_mailing_event_delivered",
      'select' => array(
  ...
  _mailingstats_cron_event($params, 'Total clicks',
    array (
  ...
  _mailingstats_cron_event($params, 'Gmail Total clicks',
    array (
      'event_type' => "civicrm_mailing_event_trackable_url_open",
      'select' => array(
  ...
  _mailingstats_cron_event($params, 'Unique clicks',
    array (
      'event_type' => "civicrm_mailing_event_trackable_url_open",
      'select' => array(
  ...
  _mailingstats_cron_event($params, 'Gmail Unique clicks',
    array (
      'event_type' => "civicrm_mailing_event_trackable_url_open",
      'select' => array(
  ...
  _mailingstats_cron_event($params, 'forwarded',
    array (
  ...
  _mailingstats_cron_event($params, 'Unsubscribed',
    array (
  ...
  _mailingstats_cron_event($params, 'Bounced',
    array (
  ...
  _mailingstats_cron_event($params, 'Opened',
    array (
  ...
  _mailingstats_cron_event($params, 'Gmail Opened',
    array (
      'event_type' => "civicrm_mailing_event_opened",
      'select' => array(
  ...
  _mailingstats_cron_trackable_urls($params);
  _mailingstats_cron_contribution_page_clicks($params);
  _mailingstats_cron_contributions($params);
  _mailingstats_cron_send_rate($params);
  _mailingstats_cron_done($params);
  return true;
}
...
function _mailingstats_do_query($params, $sql) {
  global $options;
  switch( $params['mode']) {
    case 'print':
      print $sql . "\n\n";
...
function _mailingstats_record_status($params, $doing) {
  $sql = <<<END
INSERT INTO agc_report_mailing_stats_performance (doing) VALUES ("${doing}");
END;
  _mailingstats_do_query($params, $sql);
}
...
function _mailingstats_cron_event($params, $task, $spec) {
...
END;
  ...
  _mailingstats_record_status($params,$task);
  $sql = <<<END
...
function _mailingstats_cron_db_setup($params, $recreate = 0) {
...
function _mailingstats_cron_mailing($params) {
  _mailingstats_record_status($params,"Getting stuff from mailing table");
  $sql = <<<END
...
function _mailingstats_cron_job_timing($params) {
_mailingstats_record_status($params,"Collect info on start and end times");
...
function _mailingstats_cron_recipients($params, $task, $gmail_only=0) {
...
function _mailingstats_cron_trackable_urls ($params) {
_mailingstats_record_status($params,"Trackable urls");
...
function _mailingstats_cron_contribution_page_clicks ($params) {
_mailingstats_record_status($params,"contribution_page_clicks");
  $sql = <<<END
UPDATE agc_report_mailing_stats
LEFT JOIN (
  SELECT j.mailing_id AS mailing_id,
...
function _mailingstats_cron_contributions ($params, $hours = 48) {
_mailingstats_record_status($params,"contributions_${hours}hrs");
  $sql = <<<END
UPDATE agc_report_mailing_stats
LEFT JOIN (
SELECT 
...
function _mailingstats_cron_send_rate ($params) {
  _mailingstats_record_status($params,"Calculating Send Rate");
...
function _mailingstats_cron_done ($params) {
  $sql = <<<END
